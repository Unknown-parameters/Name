...
import random
#生成图片
img = np.zeros([120, 400, 3], np.uint8)+255
#生成随机数字,文字
def v_code():
    res = []
...
def print_chinese(img,num,text):
    cv2.putText(img , num, (10, 40), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 1)
    cv2img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # cv2和PIL中颜色的hex码的储存顺序不同
    pilimg = Image.fromarray(cv2img)
    # PIL图片上打印汉字
    draw = ImageDraw.Draw(pilimg) # 图片上打印
    font = ImageFont.truetype("simhei.ttf", 15, encoding="utf-8") # 参数1：字体文件路径，参数2：字体大小
    draw.text((10, 60), text, (0, 0, 0), font=font) # 参数1：打印坐标，参数2：文本，参数3：字体颜色，参数4：字体
    # PIL图片转cv2 图片
